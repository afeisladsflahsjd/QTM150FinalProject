...
-   This histogram shows us the number of counties with corresponding amounts of farm land
...
-   This histogram shows us the counties with their corresponding population densities
-   It was decided that population density \<100 per mile\^2 was a good parameter as it encapsulates most counties
...
-   Given the individual distributions, as well as the scatter plot, we decided threshold values of \<100 people/mi\^2 for population density, and \>60% farmland would be ideal ranges that would give a broad range of candidate counties
...
ggplot(combined_data%>%filter(!is.na(candidate)), 
  ...
  geom_density_ridges(alpha=0.7, show.legend=FALSE, na.rm = TRUE, bandwidth=0.61) +
  labs(
...
ggplot(combined_data%>%filter(!is.na(candidate)), 
  ...
  geom_density_ridges(alpha=0.7, show.legend=FALSE, na.rm = TRUE, bandwidth = 2.03) +
  labs(
...
ggplot(combined_data%>%filter(!is.na(candidate)), 
  ...
  geom_density_ridges(alpha=0.7, show.legend=FALSE, na.rm = TRUE, bandwidth = 2.11) +
  labs(
...
ggplot(combined_data%>%filter(!is.na(candidate)), 
  ...
  geom_density_ridges(alpha=0.7, show.legend=FALSE, na.rm = TRUE, bandwidth=0.00572) +
  labs(
...
-   DON'T go to Monroe County, Florida and get out of New York City if you're from there.
-   Head for Bailey County in Texas, Parmer County in Texas, Castro County in Texas, Neosho County in Kansas, and Lawrence County in Arkansas.
